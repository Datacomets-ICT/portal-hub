...
const BookingFlow = ({ setPage, empId, onComplete }) => {
  ...
  const submit = () => {
    ...
    const pickup = data.pickupMode === "list"
      ? data.pickup
      : { name: data.pickupCustom.name, detail: data.pickupCustom.detail, map: data.pickupCustom.map };
    const dropoff = data.dropoffMode === "list"
      ? data.dropoff
      : { name: data.dropoffCustom.name, detail: data.dropoffCustom.detail, map: data.dropoffCustom.map };
    setBookingId(id);
    setSubmitted(true);
  ...
  return (
    <div className="anim-fade" style={{paddingBottom:96}}>
      ...
      <div style={{
        ...
        <Btn variant="ghost" onClick={back} icon={<Ico.ArrowLeft/>}>ยกเลิก</Btn>
