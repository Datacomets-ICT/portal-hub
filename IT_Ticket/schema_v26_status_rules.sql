-- ============================================================
-- Phase 26: Enforce status transition rules
--   - Admin เปลี่ยนกลับเป็น "เปิด Ticket" ไม่ได้ (ถ้าสถานะผ่านไปแล้ว)
--   - Admin set "ยกเลิก" ไม่ได้ (User เท่านั้น ผ่าน cancel_my_ticket)
-- ============================================================
...
begin
  ...
  -- 🚫 Status Rules
  -- ============================================================
  -- Rule 1: Admin เปลี่ยนเป็น "ยกเลิก" ไม่ได้ (ต้องให้ User ยกเลิกผ่าน cancel_my_ticket)
  if new_status = 'ยกเลิก' and old_row.status <> 'ยกเลิก' then
    return json_build_object('success', false,
      'message', 'ไม่สามารถยกเลิก Ticket ได้ — ต้องให้ผู้แจ้งยกเลิกเอง');
  end if;

  -- Rule 2: Admin ย้อนกลับเป็น "เปิด Ticket" ไม่ได้ ถ้าสถานะผ่านไปแล้ว
  if new_status = 'เปิด Ticket' and old_row.status not in ('เปิด Ticket') then
    return json_build_object('success', false,
      'message', 'ไม่สามารถย้อนสถานะกลับเป็น "เปิด Ticket" ได้ (ต้องให้ผู้แจ้งกดเปิดใหม่เอง)');
  end if;

  -- Rule 3: เปลี่ยนสถานะจาก "ปิดงานแล้ว" ไม่ได้ (final state)
  if old_row.status = 'ปิดงานแล้ว' and new_status <> 'ปิดงานแล้ว' then
    return json_build_object('success', false,
      'message', 'Ticket ปิดงานแล้ว แก้ไขไม่ได้');
  end if;
...
-- ============================================================
-- ทดสอบ:
--   - Login admin → แก้ ticket ที่ status = "กำลังดำเนินการ" → เลือก "เปิด Ticket"
--     → ควร return error "ไม่สามารถย้อนสถานะกลับ..."
--   - Login admin → เลือก "ยกเลิก" → ควร error "ไม่สามารถยกเลิก..."
-- ============================================================
