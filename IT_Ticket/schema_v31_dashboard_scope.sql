-- ============================================================
-- Phase 31: Dashboard byAdmin + adminPerf — filter admin by scope
--   Sr.Mgr Comets (แชมป์) จะไม่เห็น admin "วี" (ICT) ใน byAdmin chart
-- ============================================================
...
begin
  ...
  return json_build_object(
    ...
    -- byAdmin: join admin text -> employees to filter out admins outside scope
    'byAdmin', (
      select coalesce(json_agg(row_to_json(x) order by x.total desc), '[]'::json)
      from (
        select coalesce(t.admin,'ยังไม่มอบหมาย') as admin,
               count(*) as total,
               count(*) filter (where t.status in ('ดำเนินการเรียบร้อย','ปิดงานแล้ว')) as resolved
        from tickets t
        left join employees e on e.employee_id = t.employee_id
        ...
        where case v_role
          ...
          -- Filter out admins outside our scope (if senior_manager)
          and case v_role
                when 'senior_manager' then (
                  coalesce(t.admin,'') = ''
                  or adm.scope_company = v_scope
                  or adm.role = 'manager'
                )
                else true
              end
...
-- ============================================================
-- ทดสอบ: Login แชมป์ → Dashboard → byAdmin ไม่ควรมี "วี" แล้ว
-- ============================================================
